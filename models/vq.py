...
class VQ(nn.Module):
    def __init__(
        ...
        self.register_buffer('vq_embedding', torch.randn(self._num_embeddings, self._embedding_dim)) #normalized codebook
        self.register_buffer('vq_embedding_output', torch.randn(self._num_embeddings, self._embedding_dim)) # output codebook
        self.register_buffer('vq_ema_cluster_size', torch.zeros(num_embeddings))
        self.register_buffer('vq_ema_w', torch.randn(self._num_embeddings, self._embedding_dim))
    ...
    def update(self, x):
        ...
        self.vq_ema_cluster_size.data = self.vq_ema_cluster_size * self._decay + (1 - self._decay) * torch.sum(encodings, 0)

        # Laplace smoothing of the cluster size
        n = torch.sum(self.vq_ema_cluster_size.data)
        self.vq_ema_cluster_size.data = (self.vq_ema_cluster_size + 1e-5) / (n + self._num_embeddings * 1e-5) * n
        ...
        self.vq_embedding.data = self.vq_embedding.data * self._decay + (1 - self._decay) * dw / self.vq_ema_cluster_size.unsqueeze(1)
        '''
